...
tmp.his$mids[length(tmp.his$mids)]
length(tmp.his$breaks)
tmp.his$breaks[7]

data.frame(x = tmp.sim[[ii]]$b1h) %>%
  ggplot(aes(x = x)) +
  geom_histogram(binwidth = 0.2, center = 3.2, fill = "blue", color = "white") +
...
data.frame(x = tmp.sim[[ii]]$b1h) %>%
  ...
  stat_bin(aes(y = ..density..), bins = length(tmp.his$mids) - tmp.his$breaks[1], fill = "blue", color = "white") +
  scale_x_continuous(limits = c(-5, 5)) +
  scale_y_continuous(limits = c(0, 0.5)) +
  labs(title = "",
       x = "", y = "Relative Frequency")
