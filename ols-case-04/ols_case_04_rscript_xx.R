...
bet_hat_sim_fun <- function(RR, NN,
  ...
  for (ii in 1:RR) {
    ...
    X <- runif(NN, min = 0-X.int/2, max = 0+X.int/2)
...
pb = txtProgressBar(min = 0, max = length(X.int.vec), initial = 0) 

for (ii in 1:length(X.int.vec)) {
  ...
  for (jj in 1:length(u.sd.vec)) {
    ...
    plt.nam <- paste("plot_01_Xint", X.int, "_usd", u.sd, ".svg", sep = "")
    svg(plt.nam) 
    ...
    plt.nam <- paste("plot_02_Xint", X.int, "_usd", u.sd, ".svg", sep = "")
    svg(plt.nam) 
    ...
    plt.nam <- paste("plot_03_Xint", X.int, "_usd", u.sd, ".svg", sep = "")
    svg(plt.nam) 
