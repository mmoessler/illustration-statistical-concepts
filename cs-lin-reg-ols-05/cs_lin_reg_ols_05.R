...
needed.packages <- c("MASS", "scales", "Cairo", "svglite", "ellipse")
...
result <- foreach(ind = 1:nrow(tmp.grd),
                  ...
                  .options.RNG = 12345) %dorng% {
                    ...
                    # plot no 02 confidence ellipse of beta combinations ----
                    ...
                    coef_names <- c("x1", "x2")
                    coef_indices <- match(coef_names, names(coefficients))
                    coef_values <- coefficients[coef_indices]

                    ellipse_data <- ellipse::ellipse(vcov_matrix[coef_indices, coef_indices], centre = coef_values, level = 0.95)
                    plot(ellipse_data,
                         type = "n",
                         xlab = "Coefficients of X1",
                         ylab = "Coefficients of X2",
                         main = "Joint Distribution of Coefficients")

                    # Add points for the estimated coefficients
                    points(coef_values[1], coef_values[2], col = "blue", pch = 16)

                    # Fill the 95% confidence area
                    polygon(ellipse_data, col = scales::alpha("red", 0.3), border = NA)
